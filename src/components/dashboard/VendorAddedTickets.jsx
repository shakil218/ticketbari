...
export default function VendorAddedTickets({ totalTickets }) {
  ...
  const handleDelete = async (ticketId) => {
    if (!confirm("Are you sure you want to permanently delete this ticket layout?")) return;

    try {
      // await deleteTicketAction(ticketId);
      
      setTickets((prev) => prev.filter((ticket) => ticket._id !== ticketId));
      toast.success("Ticket listing removed successfully.");
    } catch (error) {
      console.error(error);
      toast.error("Failed to delete ticket asset.");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Title Block Layout */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">My Added Tickets</h1>
          <p className="text-sm text-default-500 mt-1">
            View, modify, and track validation reviews for your uploaded route listings.
          </p>
        </div>
        ...
        {tickets.length === 0 ? (
          <Card className="p-12 text-center border border-dashed border-divider flex flex-col items-center justify-center">
            ...
            <p className="text-base font-medium text-default-700">No Tickets Created Yet</p>
            <Button as={Link} href="/dashboard/vendor/tickets/add" className="bg-[#006B4A] text-white rounded-xl mt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tickets.map((ticket) => {
              ...
              return (
                <Card key={ticket._id} className="overflow-hidden border border-divider shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between h-full bg-content1 rounded-2xl">
                  <div>
                    ...
                    <div className="relative w-full h-44 bg-default-100">
                      {ticket.imageUrl ? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center text-default-400">
                          ...
                          <span className="text-xs uppercase tracking-wider font-semibold">No Preview Cover</span>
                        </div>
                      )}
                      
                      {/* Floating Verification Badge Status */}
                      <div className="absolute top-3 right-3 z-10">
                        <Chip size="sm" color={statusConfig.color} variant="flat" className="font-bold uppercase tracking-wider text-[10px] px-2.5">
                          {statusConfig.text}
                        </Chip>
                      </div>
                    </div>
                    ...
                    <div className="p-5 space-y-4">
                      <div>
                        <h3 className="font-bold text-slate-800 dark:text-white line-clamp-1 text-lg mb-1">{ticket.title}</h3>
                      ...
                      {ticket.perks && ticket.perks.length > 0 && (
                        <div className="flex flex-wrap gap-1 pt-1">
                          {ticket.perks.map((perk, idx) => (
                            <span key={idx} className="text-[10px] bg-default-50 text-default-500 border border-default-200 px-1.5 py-0.5 rounded flex items-center gap-0.5 font-medium">
                      ...
                      <div className="text-xs text-default-400 pt-1 flex items-center gap-1.5">
                        ...
                        <span>{new Date(ticket.departureTime).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-divider p-4 bg-default-50/50 mt-auto">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center text-default-500 text-xs gap-1">
                        ...
                        <span>Available Units: <strong className="text-default-800 dark:text-default-200">{ticket.quantity}</strong></span>
                      </div>
                      <div className="flex items-center text-emerald-600 dark:text-emerald-400 font-extrabold text-lg">
                        <DollarSign className="w-4 h-4 -mr-0.5" />
                        <span>{ticket.price.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                      <Button
                        as={isRejected ? "button" : Link}
                        href={isRejected ? undefined : `/dashboard/vendor/tickets/update/${ticket._id}`}
                        size="sm"
                        variant="bordered"
